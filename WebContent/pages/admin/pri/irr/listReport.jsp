...
<body>
	<div class="so_main">
		...
		<div id="search_div" style="display: none;">
			...
			<div class="form2">
				<form method="post" action="${ctx}/admin/pri/irr_listReport.action"
					id="searchForm">
					<dl class="lineD">
						<dt>IRR编号：</dt>
						<dd>
					...
					<div class="page_btm">
						<input type="submit" class="btn_b" value="确定" /> <input
							type="button" class="btn_b" id="clear" value="清空" />
					</div>
				</form>
			</div>
		</div>
					...
					<td>
						...
						<c:if test="${1 eq entity.cldType }">偏差</c:if>
				      	...
				      	<c:if test="${5 eq entity.cldType }">报废</c:if>
							...
							<td><a href='javascript:void(0);' class='fn edit'
								id="${entity.id }">查看详细</a> <c:if
								...
								</c:if><c:if test="${'QEJL' eq SESSION_ADMIN_USER.role.code&&entity.sqeFile>0&&entity.closed==0}">
									<br />
									<a href='javascript:void(0);' class='fn close'
										id="${entity.id }">关闭</a>
								</c:if> <c:if test="${entity.state == 2 }">
									<c:if
										test="${'QEZG' eq SESSION_ADMIN_USER.role.code ||'QEJL' eq SESSION_ADMIN_USER.role.code }">
										<a href='javascript:void(0);' class='fn edit'
											id="${entity.id }">撤销</a>
									</c:if>
								</c:if></td>
		...
		<div class="Toolbar_inbox">
			<p:page url="${ctx}/admin/pri/irr_listReport.action"
				pageInfo="${pageInfo }"
						...
						function() {
							...
							$("a.edit")
									.click(
											function() {
												var id = $(this).attr('id');
												window.location.href = "${ctx}/admin/pri/irr_edit.action?isShow=true&pageIndex=${pageInfo.pageIndex}&model.id="
														+ id;
												return false;
											});
