...
public class FileUtil {
	 ...
	 */
	public static String[] splitFileByEachFileLines(String orgDir, String originFile, String destDir, String ext,
			int count) {
		...
		try {
			...
			while ((tmpLine = reader.readLine()) != null) {
				...
				if (line == lineCountInEveryFile // 每个文件的行数到了
					...
					// 将内容写入文件
					String filename = destDir + System.currentTimeMillis() + "_" + fileStep + ext;
			...
			if (tmpContent.length() > 0) {
				// 将内容写入文件
				String lastFileName = destDir + System.currentTimeMillis() + "_" + fileStep + ext;
	 ...
	 */
	public static Boolean renameTo(String srcFile, String destDir, String destfile) {
	...
	public static void writeFileByString(String filePath, String fileContent, String encoding, boolean append) {
		...
		try {
			if (filePath == null || fileContent == null || fileContent.length() <= 0) {
				log.error("into writeFileByString [filePath=" + filePath + ",filePath=" + fileContent
						+ "] is null return!!!");
				return;
			}
			...
			if (encoding == null || encoding.trim().length() <= 0) {
				...
				out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath, append),
						encoding)), true);
			}
	 ...
	 * @throws Exception
	 */
	public static void writeFileToString(String filePath, String fileContent, String encoding, boolean append)
			throws Exception {
		PrintWriter out = null;
		try {
			if (filePath == null || fileContent == null || fileContent.length() <= 0) {
